...
class SummaryRekapController extends BaseController
{
    ...
    public function create()
    {
        ...
        // $transferData = $transaksiModel->where('created_at >=', $start_periode)->where('created_at >=', $end_periode)->where('payment_method', 'Non Tunai')->first();
        $transferData = $transaksiModel->where('DATE(created_at)', $start_periode)->where('payment_method', 'Non Tunai')->get()->getResultArray();
        $tunaiData = $transaksiModel->where('DATE(created_at)', $start_periode)->where('payment_method', 'Cash')->get()->getResultArray();

        $totalTransferPrice = 0;
        foreach ($transferData as $data) {
            $totalTransferPrice += $data['total_price'];
        }

        $totalTunaiPrice = 0;
        foreach ($tunaiData as $data) {
            $totalTunaiPrice += $data['total_price'];
        }
        ...
        return view('pages/summary/create', [
            ...
            'totalTunaiPrice' => $totalTunaiPrice
        ]);
    }
